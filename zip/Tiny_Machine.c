...
typedef enum {
   ...
   opLT,     /* RR     reg(r) = reg(s)<reg(t) */
   opLE,     /* RR     reg(r) = reg(s)<=reg(t) */
   opGT,     /* RR     reg(r) = reg(s)>reg(t) */
   opGE,     /* RR     reg(r) = reg(s)>=reg(t) */
   opEQ,     /* RR     reg(r) = reg(s)==reg(t) */
   opNE,     /* RR     reg(r) = reg(s)!=reg(t) */
   opAND,    /* RR     reg(r) = reg(s)&&reg(t) */
   opOR,     /* RR     reg(r) = reg(s)||reg(t) */
   opNOT,     /* RR     reg(r) = !reg(s), t is ignored */
...
char * opCodeTab[]
        = {"HALT","IN","OUT","ADD","SUB","MUL","DIV","LT","LE","GT","GE","EQ","NE", "AND", "OR", "NOT","????",
            /* RR opcodes */
           "LD","ST","????", /* RM opcodes */
...
{ INSTRUCTION currentinstruction  ;
  ...
  { /* RR instructions */
    ...
    /***********************************/
      ...
      break;
	case opLT :  reg[r] = reg[s] < reg[t] ;   break;
	case opLE :  reg[r] = reg[s] <= reg[t] ;   break;
	case opGT :  reg[r] = reg[s] > reg[t] ;   break;
	case opGE :  reg[r] = reg[s] >= reg[t] ;   break;
	case opEQ :  reg[r] = reg[s] == reg[t] ;   break;
	case opNE :  reg[r] = reg[s] != reg[t] ;   break;
	case opAND:  reg[r] = reg[s] && reg[t] ;   break;
	case opOR :  reg[r] = reg[s] || reg[t] ;   break;
	case opNOT:  reg[r] = !reg[s];             break;
    /*************** RM instructions ********************/
